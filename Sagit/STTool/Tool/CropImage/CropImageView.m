...
@property(nonatomic, assign) BOOL editScaleSize;
...
-(CropImageView *)setPara:(UIImage *)image scaleSize:(CGSize)scaleSize editScaleSize:(BOOL)editScaleSize
{
    ...
    self.editScaleSize=editScaleSize;
    self.bigImageView.image = self.targetImage;
    
    self.clipWidth =SCREEN_WIDTH;//SCREEN_WIDTH;//
    self.clipHeight =self.clipWidth * scaleSize.height/scaleSize.width;
...
- (void)initUI {
    ...
    [self.cropView frame:STFullRect];
...
- (void)initWidthHeight
{
    ...
    [[self.bigImageView frame:frame] toCenter];
...
-(void)addAllGesture
{
    ...
    // 拖动手势
    CropPanGestureRecognizer *panGesture = [[CropPanGestureRecognizer alloc] initWithTarget:self action:@selector(handleDynamicPanGesture:) inview:self.cropView];
    [self.cropView addGestureRecognizer:panGesture];
   
}

-(void)handleDynamicPanGesture:(CropPanGestureRecognizer *)panGesture
{
...
- (void)setUpCropLayer
{
    ...
    CropClipAreaLayer * layer = [[CropClipAreaLayer alloc] init];
